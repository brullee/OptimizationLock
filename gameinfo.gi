...
// --- 1. Field of View --- 
citadel_camera_hero_fov "90"                                   // Use this if you want FOV between 75-90. [def: "75"] 
//r_aspectratio "0"                                            // Use this if you want a more flexible range. 1.75=80fov | 2.15=90fov | 2.49=100fov (this is affected by the command above). [def: "0"]
...
citadel_player_outline_fade_range_max "1400"                    // Adjusts the distance at which outlines stay rendered. [def: "1400"]
r_citadel_selection_outline2_alpha "4"                          // Adjusts the opacity of all outlines. [def: "0.8"]
r_citadel_selection_outline2_fade_pow "1.5"                		// 0 = Outline entire entity, 0 < (larger than 0) Fade entity outline from bottom. [def: "1.5"]
r_citadel_selection_outline2_offset "0.3"						// 0 > Moves outline closer to entity, 0 = Leaves no space between outline and entity, 0 < Moves outline away from entity. [def: "0.3"]

// --- 3. HUD --- 
citadel_unit_status_use_new "0"                                 // This uses an alternate Health Bar, to use old Health Bar change "true" to "false". [def: "0"]
citadel_unit_status_use_v2 "1"                                  // This uses a new Health Bar, to use old Health Bar change "true" to "false". [def: "0"]
...
// --- 7. Object Culling ---
r_size_cull_threshold "0.7"                                     // *Culls small objects sooner based on screen size threshold (higher = more culling + more pop-ins/outs). [def: "0.8"]
...
// ================== MODELS ================= 
r_citadel_npr_outlines "1"                                      // Renders stylistic outlines. [def: "1"]
r_citadel_npr_outlines_max_dist "500"                           // Limits the distance at which stylistic outlines render on enteties (lower = need to be closer for some details to be rendered). [def: "1000"]
...
cl_interp "0"                                                   // Client-side interpolation time (smoothing delay) for rendering other players/entities. [def: 0]
cl_interp_ratio "1"                                             // Multiplier that affects interpolation time (often cl_interp_ratio / cl_updaterate). [def: "0"]
cl_smoothtime "0.2"                                             // Smooth client's view after prediction error over this many seconds (Lower = snappier but more abrupt, higher = smoother but floaty). [def: "0.2"]
